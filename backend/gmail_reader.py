...
def search_replies_from(creds, from_email: str, after_iso: Optional[str] = None,
    ...
    """Return message metadata for emails in the connected user's mailbox that
    were received from `from_email` (optionally after a given timestamp).

    Each returned dict: {id, thread_id, snippet, internal_date, subject, body_text}.
    Body is the *decoded* plain-text (or HTML->text) content of the message,
    truncated to 20000 chars. Returns [] on any error to keep the scanner loop
    resilient.
    """
    if not from_email:
        return []
    q_parts = [f'from:{from_email}']
    if after_iso:
