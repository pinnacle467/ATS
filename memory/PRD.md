...
- **Google Calendar/Meet setup guidance given** (not yet configured): shared the exact Google Cloud Console links (Calendar API, Gmail API, Meet API enable pages; OAuth consent screen; credentials page) and the required redirect URI format (`<APP_BASE_URL>/api/oauth/calendar/callback`) with the user so they can create their own OAuth client. `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` still need to be added to `.env` once the user has them.
